...
export default function SignUpPage() {
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      // The callback page will handle creating the user profile
    } catch (err: any) {
  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    ...
    try {
      // Create account with real Supabase
      const { data: authData, error: authError } = await supabaseHelpers.signUp(
        formData.email,
        formData.password,
        { name: formData.name }
      );
